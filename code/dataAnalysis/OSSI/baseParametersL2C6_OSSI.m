...
% time axis
tStart = DBGetDatabaseEntry('instruments','L2C6_OSSI','timeIN',MET2sedmextime([2021 9 10 0 0 0])); % Dates are not used, output is start and end time
tEnd = DBGetDatabaseEntry('instruments','L2C6_OSSI','timeOUT',MET2sedmextime([2021 9 10 0 0 0]));
...
L2C6_OSSI_par = NaN([Nt 9]);
L2C6_OSSI_par(:,1) = tAxis;
...
for t = 1:Nt
   ...
   try
       Fs = DBGetDatabaseEntry('instruments','L2C6_OSSI','sampleFrequency',tAxis(t));
       data = DBGetData('L2C6_OSSI',[tAxis(t) tAxis(t)+dt-1/Fs],{'p'});
   catch
       continue;
   end 
   ...
   if sum(isnan(p)) ~= length(p)
      ...
      zInstrumentMANUAL = DBGetInstrumentHeightManual('L2C6_OSSI',{L2C6_OSSI_par(t,1)});
      ...
      if (sum((p + min(zPT,0)) > 0.04) > 0.99*length(p)) && ~isnan(zPT)
         ...
         L2C6_OSSI_par(t,2) = h;                                       % total waterdepth 
         L2C6_OSSI_par(t,3) = zPT;                                     % z
         L2C6_OSSI_par(t,4) = 4*std(sseHF);                            % Hm0 HF
         L2C6_OSSI_par(t,5) = 1/(HF(id));                              % peak period
         L2C6_OSSI_par(t,6) = skewness(sseHF);                         % skewness HF
         L2C6_OSSI_par(t,7) = skewness(imag(hilbert(sseHF)));          % asymmetry HF
         L2C6_OSSI_par(t,8) = 4*std(sseLF);                            % Hm0 LF

         id = find(f >= HFreq(1) & f <= HFreq(2));
         Tm_10 = trapz(f(id),(f(id).^(-1)).*(S(id))) ./ trapz(f(id),S(id));  % Tm_10
         L2C6_OSSI_par(t,9) = Tm_10;                                   % Tm_10
      end
   end
...
save([basePath 'results' filesep 'OSSI' filesep 'baseParameters_L2C6_OSSI.mat'], 'L2C6_OSSI_par');
