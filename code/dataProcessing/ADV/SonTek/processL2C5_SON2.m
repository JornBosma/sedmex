% Process all SonTek ADV raw data from L2C5_SON2
...
% where is the data?
rawDataPath = [basePath 'dataRaw' filesep 'ADV' filesep 'L2C5 SonTek Hydra #B361' filesep 'raw' filesep];
...
for i = 1:nRawFiles
    ...
    if i < 28
        ...
        dataRaw = importdata([rawDataPath, fileNumber, '.ts1']);
        % lees header; gaat dit fout, dan moeten er ergens spaties worden toegevoegd.
        header = importdata([rawDataPath, fileNumber, '.hd1']);
    else
    ...
    for b = 1:Nbursts
        ...
        when(:,2) = mon;
        when(:,3) = day;
        if L2C5_SON2.summerTime % SEDMEX campaign should be in summertime
            when(:,4) = hour;
        else
