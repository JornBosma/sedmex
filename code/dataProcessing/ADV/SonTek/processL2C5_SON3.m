% Process all SonTek ADV raw data from L2C5_SON3
...
% where is the data?
rawDataPath = [basePath 'dataRaw' filesep 'ADV' filesep 'L2C5 SonTek Hydra #B362' filesep 'raw' filesep];
...
for i = 23:nRawFiles
    ...
    dataRaw = readmatrix([rawDataPath, fileNumber, '.ts1'], "FileType", "text", "NumHeaderLines", 1);
    ...
    for b = 1:Nbursts
        ...
        when(:,2) = mon;
        when(:,3) = day;
        if L2C5_SON3.summerTime % SEDMEX campaign should be in summertime
            when(:,4) = hour;
        else
